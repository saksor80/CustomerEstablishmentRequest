...
{
    public class CustomerEstablishmentRequestResultRejected //Sets model for CustomerEstablishmentResultRejected
    {
        ...
        public int UpdateRecord()   //Updates record in CustomerInformation-table
        {
            ...
            {
                string query = "UPDATE CustomerInformation SET HandledDate = @HandledDate, RequestStatus = @RequestStatus, Comment = @Comment WHERE RequestID = @RequestID";  //Sets sql command string
